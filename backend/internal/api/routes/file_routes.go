...
func FileRoutes(route *gin.RouterGroup, fileHandler *handlers.FileHandler, minioClient *minio.Client) {
	...
	{	
		...
		file.GET("/:fileCode/download", middlewares.JWTMiddleware(), middlewares.MinIOMiddleware(fileHandler.FileService, minioClient), fileHandler.FileDownload) // TODO: implement reverse proxy for download
